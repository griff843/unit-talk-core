...
async function collectMetricsSample(apiPort: number): Promise<MetricsSnapshot> {
  ...
  try {
    const response = await fetch(`http://localhost:${apiPort}/api/metrics/ingestion?window=5`);
...
async function runCanaryMonitoring(
  ...
  const totalPromoted = validSamples.reduce((sum, s) => sum + (s.data?.promoted_5min || 0), 0);
  const avgPromotionsPerWindow = validSamples.length > 0 ? totalPromoted / validSamples.length : 0;
  
  // Health scoring
  let healthScore = 0;
  
  // 40% - API availability
  const apiAvailability = samples.length > 0 ? (validSamples.length / samples.length) : 0;
  ...
  if (validSamples.length >= 3) {
    ...
    const mean = promotionRates.reduce((a, b) => a + b, 0) / promotionRates.length;
    const variance = promotionRates.reduce((sum, rate) => sum + Math.pow(rate - mean, 2), 0) / promotionRates.length;
    const consistency = variance < mean ? 20 : Math.max(0, 20 - (variance - mean));
  ...
  if (!ok) {
    if (healthScore < 70) issues.push(`Health score ${healthScore.toFixed(1)} below threshold (70)`);
    if (totalPromoted === 0) issues.push('Zero promotions detected - workflow may be broken');
    if (apiAvailability < 0.8) issues.push(`API availability ${(apiAvailability * 100).toFixed(1)}% below threshold (80%)`);
  }
...
async function main() {
  try {
    ...
    const report = await runCanaryMonitoring(durationMinutes, apiPort, intervalSeconds);
