...
async function testEnvironmentVariables(): Promise<TestResult[]> {
  ...
  try {
    ...
    const requiredTests = [
      { key: 'DATABASE_URL', value: config.DATABASE_URL, test: 'Database URL configured' },
      { key: 'SUPABASE_URL', value: config.SUPABASE_URL, test: 'Supabase URL configured' },
      { key: 'SUPABASE_ANON_KEY', value: config.SUPABASE_ANON_KEY, test: 'Supabase anon key configured' },
      { key: 'SUPABASE_SERVICE_KEY', value: config.SUPABASE_SERVICE_KEY, test: 'Supabase service key configured' },
    ];
    ...
    const featureTests = [
      ...
      { key: 'PUBLISH_TO_DISCORD', value: config.PUBLISH_TO_DISCORD, expected: false },
    ];
