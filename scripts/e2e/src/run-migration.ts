...
async function testMigrations(): Promise<TestResult[]> {
  ...
  try {
    ...
    const dryRunResult = await runCommand('npm', ['run', 'migrate:dry-run', '--workspace=packages/db']);
    ...
    const upResult = await runCommand('npm', ['run', 'migrate:up', '--workspace=packages/db']);
    ...
    const idempotencyResult = await runCommand('npm', ['run', 'migrate:up', '--workspace=packages/db']);
