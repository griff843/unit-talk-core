...
export function deduplicateItems(
  ...
  return {
    ...
    statistics: {
      ...
      deduplicationRate:
        items.length > 0 ? duplicateItems.length / items.length : 0,
    },
  };
}
 ...
 */
export function createStatisticsSummary(items: ProcessedFeedItem[]): {
  totalItems: number;
  averageScore: number;
  ...
  const averageScore =
    items.reduce((sum, item) => sum + item.processingScore, 0) / totalItems;
