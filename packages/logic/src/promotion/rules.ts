...
import { PromotionError, PROMOTION_CONSTANTS } from './types.js';
...
export function calculateEligibilityFactors(
  ...
  const ageHours =
    (currentTime.getTime() - insertedAt.getTime()) / (1000 * 60 * 60);
...
export function filterEligibleCandidates(
  ...
  for (const candidate of candidates) {
    ...
    if (
      candidate.eligibilityFactors.qualityScore < config.minQualityThreshold
    ) {
      rejected.push({
        candidate,
    ...
    // Check age limit
    const ageHours =
      (Date.now() - candidate.insertedAt.getTime()) / (1000 * 60 * 60);
...
export function validatePromotionResult(result: PromotionResult): void {
  if (
    ...
    result.selectedCandidates.length + result.rejectedCandidates.length
  ) {
  ...
  if (
    result.selectedCandidates.length >
    result.metadata.configUsed.maxPromotionsPerWindow
  ) {
    throw new PromotionError(
...
function assessDataQuality(data: Record<string, unknown>): number {
  ...
  score +=
    (presentFields.length / expectedFields.length) * weights.completeness;
...
function assessMarketRelevance(data: Record<string, unknown>): number {
  ...
  const keywordMatches = marketKeywords.filter(keyword =>
