...
const SAMPLE_FEED_DATA: RawFeedData[] = [
  {
    ...
    content: 'Comprehensive analysis of current market trends and predictions for the next quarter. Key indicators show positive momentum in technology sectors.',
  ...
  },
  {
    ...
    content: 'Detailed technical analysis covering moving averages, RSI indicators, and volume patterns across major indices.',
  ...
  },
  {
    ...
    content: 'Economic forecast for Q1 2025 based on current indicators and historical patterns. Expect moderate growth with some volatility.',
  ...
  },
  {
    ...
    content: 'Aggregated sentiment analysis from social media platforms showing positive outlook on technology investments.',
...
export async function executeFeedWorkflow(
  ...
  try {
    logger.info('Starting feed workflow', { config, feedDataProvided: !!feedData });
    ...
    if (limitedItems.length > 0 && !config.dryRun) {
      ...
      logger.info('DRY RUN - Would have inserted items', { count: limitedItems.length });
      insertedIds = limitedItems.map(item => `dry-run-${item.id}`);
    }
    
    // Calculate final statistics
    const averageQualityScore = limitedItems.length > 0
      ? limitedItems.reduce((sum, item) => sum + item.processingScore, 0) / limitedItems.length
      : 0;
    ...
    const result: FeedOperationResult = {
      ...
      metadata: {
        ...
        processingDuration: Date.now() - startTime,
        batchesProcessed: Math.ceil(limitedItems.length / (config.batchSize ?? 10)),
...
export async function getRawPropsStatistics(windowMinutes = 5): Promise<{
  ...
  try {
    ...
    if (processedError) {
      throw new Error(`Failed to count processed items: ${processedError.message}`);
    }
